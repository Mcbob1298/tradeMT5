"""
STRATÉGIE M5 "LE COMMANDO" - Bot Ultime
Stratégie équilibrée pour captures d'opportunités tactiques
"""
...
from ..config.config_ultimate import M5_COMMANDO_CONFIG, SYMBOL, COLORS
from ..core.risk_manager import can_open_new_trade, place_order_safely

class M5Strategy:
    def __init__(self):
        self.config = M5_COMMANDO_CONFIG
    ...
    def get_market_data(self, timeframe=mt5.TIMEFRAME_M5, bars=500):
        """Récupère les données de marché pour M5"""
        try:
    ...
    def detect_divergence(self, df):
        """Détection des divergences RSI/Prix selon les paramètres optimaux M5"""
        try:
            rsi = self.calculate_rsi(df)
            if len(rsi) < 50:
                return None, 0
                
            # Paramètres M5 optimisés (de votre calibrage)
            lookback = 7  # Plus réactif pour M5
    ...
    def analyze_signal(self):
        """Analyse complète pour signal M5"""
        try:
            ...
            # Filtre ADX - Configuration optimale M5
            ...
            # Filtre de momentum pour M5 (plus serré)
            rsi = self.calculate_rsi(df)
            current_rsi = rsi.iloc[-1]
            
            # Éviter les zones de sur-achat/sur-vente extrêmes pour M5
            if signal == "BUY" and current_rsi > 75:
                self.log(f"⚠️ Signal BUY filtré: RSI trop élevé ({current_rsi:.1f})")
                return None
            elif signal == "SELL" and current_rsi < 25:
                self.log(f"⚠️ Signal SELL filtré: RSI trop bas ({current_rsi:.1f})")
                return None
                
            current_price = df['close'].iloc[-1]
            self.log(f"🎯 Signal détecté: {signal} (Force: {strength:.1f}, ADX: {current_adx:.1f}, RSI: {current_rsi:.1f})")
    ...
    def calculate_position_size(self):
        """Calcul de la taille de position pour M5"""
    ...
    def place_trade(self, signal):
        ...
        try:
            ...
            # Calcul des niveaux selon le ratio R/R optimisé M5
            if signal["action"] == "BUY":
                order_type = mt5.ORDER_TYPE_BUY
                entry_price = symbol_info.ask
                
                # SL et TP pour M5 (équilibrés)
                sl_distance = 0.5  # 50 cents pour M5
            ...
            request = {
                ...
                "comment": f"M5_Commando_{signal['action']}_S{signal['strength']:.1f}_ADX{signal['adx']:.0f}",
    ...
    def run_strategy(self):
        """Boucle principale de la stratégie M5"""
        self.log("🚀 Démarrage de la stratégie M5")
        self.is_running = True
        
        while self.is_running:
            try:
                # Éviter les signaux trop fréquents (minimum 10 minutes entre trades pour M5)
                if time.time() - self.last_signal_time < 600:
                    time.sleep(60)
                    continue
                ...
                # Attente avant la prochaine analyse (2 minutes pour M5)
                time.sleep(120)
    ...
    def stop_strategy(self):
        """Arrête la stratégie"""
        self.is_running = False
        self.log("🛑 Stratégie M5 arrêtée")

# === FONCTION D'ENTRÉE POUR LE THREADING ===
def run_m5_strategy():
    """Fonction d'entrée pour le thread M5"""
    strategy = M5Strategy()
