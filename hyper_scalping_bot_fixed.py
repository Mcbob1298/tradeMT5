...
# Variables pour la protection contre l'overtrading du même type (fenêtre glissante 1 minute)
buy_timestamps = []  # Liste des timestamps des trades BUY
sell_timestamps = []  # Liste des timestamps des trades SELL
buy_pause_until = 0  # Timestamp jusqu'à quand les BUY sont en pause
sell_pause_until = 0  # Timestamp jusqu'à quand les SELL sont en pause
CONSECUTIVE_LIMIT = 5  # Nombre de trades du même type en 1 minute avant pause
PAUSE_DURATION = 60  # Durée de pause en secondes (1 minute)
...
def get_signal(data):
    ...
    try:
        ...
        if buy_score >= CONFIDENCE_SCORE_REQUIRED:
            tp_price = data['ask'] + (FIXED_TP_DOLLARS / 100)  # Conversion en points
            sl_price = data['ask'] - (FIXED_TP_DOLLARS * SL_MULTIPLIER / 100)
            ...
            tp_price = data['bid'] - (FIXED_TP_DOLLARS / 100)  # Conversion en points
            sl_price = data['bid'] + (FIXED_TP_DOLLARS * SL_MULTIPLIER / 100)
...
def check_consecutive_trade_pause(signal_type):
    """Vérifie si un type de trade est en pause à cause de trop de trades dans la fenêtre de 1 minute"""
    global buy_pause_until, sell_pause_until
    
    current_time = time.time()
    
    if signal_type == 'BUY':
        if current_time < buy_pause_until:
            remaining_time = int(buy_pause_until - current_time)
            logging.warning(f"⏸️ BUY en pause pour {remaining_time}s (5 BUY en moins d'1 minute)")
            return True
        return False
    elif signal_type == 'SELL':
        if current_time < sell_pause_until:
            remaining_time = int(sell_pause_until - current_time)
            logging.warning(f"⏸️ SELL en pause pour {remaining_time}s (5 SELL en moins d'1 minute)")
            return True
        return False
...
def check_and_update_trade_count(signal_type):
    """Vérifie et met à jour le compteur de trades dans la fenêtre glissante"""
    global buy_timestamps, sell_timestamps, buy_pause_until, sell_pause_until
    ...
    if signal_type == 'BUY':
        # Vérifier combien de BUY dans la dernière minute
        recent_buy_count = len(buy_timestamps)
        
        if recent_buy_count >= CONSECUTIVE_LIMIT:
            buy_pause_until = current_time + PAUSE_DURATION
            logging.warning(f"🚨 {recent_buy_count} BUY en moins d'1 minute ! Pause BUY activée pour {PAUSE_DURATION}s")
            # Reset des compteurs après activation de la pause
            buy_timestamps = []
            return False
        
        # Ajouter le nouveau trade
        buy_timestamps.append(current_time)
        logging.info(f"📊 BUY dans la dernière minute: {len(buy_timestamps)}/{CONSECUTIVE_LIMIT}")
        
    elif signal_type == 'SELL':
        # Vérifier combien de SELL dans la dernière minute
        recent_sell_count = len(sell_timestamps)
        
        if recent_sell_count >= CONSECUTIVE_LIMIT:
            sell_pause_until = current_time + PAUSE_DURATION
            logging.warning(f"🚨 {recent_sell_count} SELL en moins d'1 minute ! Pause SELL activée pour {PAUSE_DURATION}s")
            # Reset des compteurs après activation de la pause
            sell_timestamps = []
            return False
        ...
        logging.info(f"📊 SELL dans la dernière minute: {len(sell_timestamps)}/{CONSECUTIVE_LIMIT}")
...
def place_order(signal_data):
    ...
    try:
        # 1. Vérification de la pause pour ce type de trade
        signal_type = signal_data['signal']
        if check_consecutive_trade_pause(signal_type):
            return False
        
        # 2. Vérification préliminaire du nombre de trades dans la fenêtre
        clean_old_timestamps()  # Nettoyer d'abord les anciens timestamps
        current_count = len(buy_timestamps) if signal_type == 'BUY' else len(sell_timestamps)
        if current_count >= CONSECUTIVE_LIMIT:
            logging.warning(f"⚠️ {signal_type}: {current_count}/{CONSECUTIVE_LIMIT} trades dans la dernière minute - Trade bloqué")
            return False
            
        # 3. Vérification de la limite de trades par minute
        if not can_place_trade():
            return False

        # 4. Vérification et gestion intelligente de la marge
        if not check_margin_and_manage_positions():
            return False

        # 5. Utilisation du lot fixe défini dans la config
        lot = FIXED_LOT_SIZE
        ...
        # 7. Vérification des niveaux SL/TP (distance minimum MT5)
        ...
        # 8. Préparation de la requête d'ordre
        request = {
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            check_and_update_trade_count(signal_type)  # Mettre à jour les compteurs de trades dans la fenêtre glissante
            return True
        else:
        ...
        while True:
            ...
            if cycle_count % 60 == 1:  # Toutes les 60 secondes environ
                ...
                if account_info:
                    ...
                    logging.info(f"📊 BUY dernière minute: {len(buy_timestamps)}/{CONSECUTIVE_LIMIT} | SELL dernière minute: {len(sell_timestamps)}/{CONSECUTIVE_LIMIT}")
                    logging.info("=" * 60)
